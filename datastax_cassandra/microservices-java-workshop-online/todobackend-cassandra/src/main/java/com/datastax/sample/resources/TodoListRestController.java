...
public class TodoListRestController {
  ...
  public ResponseEntity<List<TaskResource>> findAll(HttpServletRequest request) {
    logger
        .info("List all task in the db: {}", rewriteUrl(request) + "?" + request.getQueryString());
  ...
  public ResponseEntity<Void> deleteAll(HttpServletRequest request) {
    logger.info("Delete all task in the db: {}",
        rewriteUrl(request) + "?" + request.getQueryString());
  ...
  /**
   * Converts {@link IllegalArgumentException} into HTTP 400 bad parameter the response body.
   *
  ...
  /**
   * Converts {@link DriverException} into HTTP 500 error codes and outputs the error message as the
   * response body.
   *
   * @param e The {@link DriverException}.
   * @return The error message to be used as response body.
  ...
  /**
   * As isSecure is still false and https is enforce by gitpod let's change it.
   */
